...
const char *post_frag =
 "struct ColorGradingPreset{vec3 gain;vec3 gamma;vec3 lift;vec3 presaturation;vec3 colorTemperatureStrength;float colorTemperature;float colorTemperatureBrightnessNormalization;};vec3 colorTemperatureToRGB(const in float temperature)"
 "{"
 ...
 "{"
   "vec4 i=vec4(0.);"
   "float s1,blr=max(texture(_MainTex,uv).w,.011);"
   "for(int t=0;t<11;t++)"
     "{"
       "float s2=s1;"
