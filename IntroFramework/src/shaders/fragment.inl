...
const char *fragment_frag =
 "#version 130\n"
 "uniform sampler2D _TextTex,_iChannel0;"
 "uniform float _iTime;"
 "uniform vec4 _iMouse,_iResolution,_DirectionalLight,_DirectionalLightColor,_PointLightPosition,_PointLightColor,_CameraPosition,_CameraLookAt,_CameraUp;"
 "uniform float _FOV,_Distance,_LensCoeff,_MaxCoC,_RcpMaxCoC,_MarchMinimum,_FarPlane,_Step,fogDensity,_TextId,_Environment,_StepIncreaseByDistance,_StepIncreaseMax,OBJMAX,OBJMIN;"
 ...
 "const float material_ID0=1,material_ID1=2,material_ID2=3,material_ID3=4,material_ID4=5,material_ID5=6,material_ID6=7,material_ID7=8,material_ID8=9;"
 ...
 "{"
   "float noiseR=(hash12(gl_FragCoord.xy+.00111*_iTime)-.5)/4.2*(.125+color.x*.25),noiseG=(hash12(gl_FragCoord.xy+.00123*_iTime)-.5)/4.2*(.125+color.y*.25),noiseB=(hash12(gl_FragCoord.xy+.00132*_iTime)-.5)/4.2*(.125+color.z*.25);"
   "return color+vec3(noiseR,noiseG,noiseB);"
 "}"
 "float GetHeightmap(vec3 p)"
 "{"
   "float hills,hill_1=16.*perlinnoise(.1*p.xz),hill_2=4.*perlinnoise(.5*p.xz),hill_3=2.*perlinnoise(p.xz),hill_4=perlinnoise(2.*p.xz);"
   "hills+=hill_1;"
   "hills+=hill_2;"
   "hills+=hill_3;"
   "hills+=hill_4;"
 ...
 "{"
   ...
   "float hill_1=8.*dat.x,hill_2=4.*dat.y,hill_3=2.*dat.z;"
   "dat=texture(_iChannel0,p.xz*4.);"
   "float hill_4=dat.x,hill_5=.5*dat.y,hill_6=.25*dat.z;"
   "hills=hill_1;"
   "hills+=hill_2;"
   "hills+=hill_3;"
   "hills+=hill_4;"
 ...
 "float hex(vec2 p)"
 "{"
   "return p.x*=1.1547,p.y+=mod(floor(p.x),2.)*.5,p=abs(mod(p,1.)-.5),abs(max(p.x*1.5+p.y,p.y*2.)-1.);"
 "}"
 "float cellTile2(in vec3 p)"
 "{"
   "vec4 d;"
   "d.x=celli(p-vec3(.81,.62,.53));"
   "p.xy=vec2(p.y-p.x,p.y+p.x)+hex(p.xy*.2);"
   "d.y=celli(p-vec3(.39,.2,.11));"
   "p.yz=vec2(p.z-p.y,p.z+p.y)+hex(p.yz*.2);"
   "d.z=celli(p-vec3(.62,.24,.06));"
   "p.xz=vec2(p.z-p.x,p.z+p.x)+hex(p.xz*.2);"
   "d.w=celli(p-vec3(.2,.82,.64));"
   "d.xy=min(d.xz,d.yw);"
 ...
 "{"
   ...
   "coord3=coord;"
   "vec4 color2=vec4(0.);"
   "for(float i=0.;i<600.;i+=8.)"
     "{"
       "coord2=coord;"
       "float tiam=i/stretch+tsky;"
       "coord2*=rotationMatrix(vec3(0.,.6,.1),tiam);"
       "float v=perlinnoise(coord2.zx*44.);"
       "color2+=min(max(coord.y*12.-5.,0.)*.2,1.)*(vec4(1.,.95,.9,1.)*(1.-v)+vec4(.9,.95,1.,1.)*v)*max(perlinnoise(coord2.zx*233.)-.67,0.)*32.*i;"
     "}"
   "color2=max(color2,vec4(0.));"
   "color2/=stretch;"
   "color+=color2;"
   "float sd=dot(normalize(vec3(-.5,-.6,.9)),coord)*.5+.5;"
   "sd=pow(sd,5.);"
   "vec3 col=mix(vec3(.05,.1,.2),vec3(.1,.05,.2),coord);"
   "color.xyz+=col/64.;"
   "return color;"
 "}"
 "vec3 skyboxSample()"
 "{"
   "vec2 uv=2.*gl_FragCoord.xy/_iResolution.xy-1.;"
   "float aspect=_iResolution.x/_iResolution.y;"
   "vec3 direction=normalize(vec3(.5*uv*vec2(aspect,1.),1.));"
   "direction*=rotationMatrix(vec3(0.,1.,0.),_iMouse.x);"
   "direction*=rotationMatrix(vec3(1.,0.,0.),_iMouse.y);"
   "vec4 fragColor=pow(skybox(direction*3.14159*2.*.1,8220.-_iTime*145.),vec4(1./2.3));"
 ...
 "{"
   "float r=a.x,height=a.y,d=length(p.xz)-r;"
 ...
 "{"
   "pos.y+=perlinnoise(pos.xz/9.+_iTime*.0317)*perlinnoise(pos.xz/12.+_iTime*.0222)*.05;"
 ...
 "{"
   ...
   "float ps=0.,vali=4.,t=_iTime*.001;"
   "pos.xz+=vec2(perlinnoise(pos.xz*.01+t)-.5,perlinnoise(pos.xz*.01+3.+t)-.5)*pos.y*4.;"
   "vec3 opos=pos;"
   "float scale=max(ceil(rand(floor(pos.xz/algorithm.xz))-.5),0.);"
   "opos.x=mod(opos.x,algorithm.x);"
   "opos.z=mod(opos.z,algorithm.z);"
   "opos.x-=algorithm.x/2.;"
   "opos.z-=algorithm.z/2.;"
   "opos.y+=scale*100.;"
   "scale+=1.;"
   "float dist=fBox(opos,vec3(.1-opos.y*.1*scale,.7,.1-opos.y*.1*scale));"
   "return dist;"
 "}"
 ...
 "{"
   "float vali=10.;"
   "vec3 opos=pos;"
   "float scale=max(ceil(rand(floor(pos.xz/vali))-.5),0.),scale2=rand(8.*floor(pos.xz/vali))+1.;"
   "opos.x=mod(opos.x,vali);"
   "opos.z=mod(opos.z,vali);"
   "opos.x+=hash1(floor(pos.z/vali)-.5)*vali*.2;"
   "opos.z+=hash1(floor(pos.x/vali)-.5)*vali*.2;"
   "opos.x-=vali/2.;"
   "opos.z-=vali/2.;"
   "opos+=scale*100.;"
   "float dist=sdCappedCylinder(opos,vec2(.51-opos.y*.03,5.5));"
   "dist-=cellTile(pos*3.)*cellTile(pos*2.)*.1;"
   "dist-=cellTile(pos)*3.3/(1.+12.*abs(pos.y+5.));"
   "return dist;"
 "}"
 ...
 "{"
   ...
   "opos.x-=vali/2.;"
   "opos.z-=vali/2.;"
   "float distance2=1e+08;"
   "distance2=min(distance2,sdSphere(opos-vec3(0.,16.+scale*100.+scale*3.,0.),2.5*scale2*(1.+pos.y*.01)));"
   "pos.y+=GetHeightmapLowPrecision(pos*algorithm)*_Environment;"
   "distance2=min(distance2,fBox(pos,vec3(1111.,12.,1111.)));"
   "distance2*=.2;"
   "distance2+=cellTile(pos/4.)*.4;"
   "distance2+=cellTile(pos/8.)*.2;"
   "distance2+=cellTile(pos*2.)*.05;"
   "distance2+=cellTile(pos*5.)*.01;"
   "return distance2;"
 "}"
 ...
 "{"
   ...
   "if(OBJMAX>0&&0>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[0],_Objects[1],_Objects[2]),vec3(_Objects[3],_Objects[4],_Objects[5]),.03),id0_distance);"
   ...
   "if(OBJMAX>15&&15>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[150],_Objects[151],_Objects[152]),vec3(_Objects[153],_Objects[154],_Objects[155]),.03),id0_distance);"
   "if(OBJMAX>17&&17>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[170],_Objects[171],_Objects[172]),vec3(_Objects[173],_Objects[174],_Objects[175]),.06),id0_distance);"
   "if(OBJMAX>18&&18>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[180],_Objects[181],_Objects[182]),vec3(_Objects[183],_Objects[184],_Objects[185]),.166),id0_distance);"
   "if(OBJMAX>19&&19>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[190],_Objects[191],_Objects[192]),vec3(_Objects[193],_Objects[194],_Objects[195]),.05),id0_distance);"
   "if(OBJMAX>20&&20>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[200],_Objects[201],_Objects[202]),vec3(_Objects[203],_Objects[204],_Objects[205]),.06),id0_distance);"
   "if(OBJMAX>21&&21>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[210],_Objects[211],_Objects[212]),vec3(_Objects[213],_Objects[214],_Objects[215]),.05),id0_distance);"
   "if(OBJMAX>22&&22>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[220],_Objects[221],_Objects[222]),vec3(_Objects[223],_Objects[224],_Objects[225]),.05),id0_distance);"
   "if(OBJMAX>24&&24>OBJMIN)"
     "id0_distance=min(sdCapsule(position,vec3(_Objects[240],_Objects[241],_Objects[242]),vec3(_Objects[243],_Objects[244],_Objects[245]),.05),id0_distance);"
   ...
   "if(OBJMAX>10&&10>OBJMIN)"
     "id6_distance=min(sdCapsule(position,vec3(_Objects[100],_Objects[101],_Objects[102]),vec3(_Objects[103],_Objects[104],_Objects[105]),.2),id6_distance);"
   "if(OBJMAX>16&&16>OBJMIN)"
     "id6_distance=min(sdCapsule(position,vec3(_Objects[160],_Objects[161],_Objects[162]),vec3(_Objects[163],_Objects[164],_Objects[165]),.06),id6_distance);"
   "if(OBJMAX>23&&23>OBJMIN)"
     "id6_distance=min(sdCapsule(position,vec3(_Objects[230],_Objects[231],_Objects[232]),vec3(_Objects[233],_Objects[234],_Objects[235]),.06),id6_distance);"
   "vec4 distID6=vec4(id6_distance,material_ID6,position.xz+vec2(position.y,0.));"
   "result=DistUnionCombine(result,distID6);"
   "float id7_distance=1e+09;"
   "if(OBJMAX>12&&12>OBJMIN)"
     ...
     "}"
   "vec4 distID7=vec4(id7_distance,material_ID7,position.xz+vec2(position.y,0.));"
   "result=DistUnionCombine(result,distID7);"
   "float id8_distance=1e+09;"
   "if(OBJMAX>13&&13>OBJMIN)"
     ...
     "}"
   "vec4 distID8=vec4(id8_distance,material_ID8,position.xz+vec2(position.y,0.));"
 ...
 "{"
   ...
   "if(hitNfo.id.x==material_ID0)"
     "mat.reflectionCoefficient=.12,mat.albedo=vec3(.622642,.537469,.537469),mat.transparency=0,mat.smoothness=.45,mat.reflectindx=.03;"
   "if(hitNfo.id.x==material_ID1)"
     "mat.reflectionCoefficient=.31,mat.albedo=vec3(.877359,.678706,.558695),mat.transparency=0,mat.smoothness=.3,mat.reflectindx=.28;"
   ...
   "if(hitNfo.id.x==material_ID3)"
     "mat.reflectionCoefficient=.27,mat.albedo=vec3(1,1,1),mat.transparency=.501961,mat.smoothness=.8,mat.reflectindx=.69;"
   "if(hitNfo.id.x==material_ID4)"
     "mat.reflectionCoefficient=.1,mat.albedo=vec3(.651775,.669811,.224324),mat.transparency=0,mat.smoothness=.1,mat.reflectindx=.2;"
   "if(hitNfo.id.x==material_ID5)"
     "mat.reflectionCoefficient=.31,mat.albedo=vec3(.471698,.259915,.0956746),mat.transparency=0,mat.smoothness=.76,mat.reflectindx=.74;"
   "if(hitNfo.id.x==material_ID6)"
     "mat.reflectionCoefficient=.54,mat.albedo=vec3(.419722,.679245,.592738),mat.transparency=0,mat.smoothness=.39,mat.reflectindx=.48;"
 ...
 "{"
   "vec3 lightDir=GetDirectionLight().direction,color=GetDirectionLight().color;"
   ...
   "backdrop+=min(max(pow(directLight,12.)*1.6,.01),1.)*1.1;"
   "backdrop*=normalize(color);"
   "if(_iMouse.x>1.)"
     "backdrop=skyboxSample().xyz;"
 ...
 "{"
   ...
   "for(int i=0;i<=maxIter;i++)"
     "{"
       ...
       "result.distanc=result.distanc+sceneDistance.x*_Step*(1.+min(_StepIncreaseByDistance,_StepIncreaseMax)*result.distanc);"
   ...
   "if(result.distanc>=_FarPlane)"
     "result.distanc=_FarPlane,result.position=ray.origin+ray.direction*result.distanc,result.id.x=0.;"
 "}"
 ...
 "{"
   ...
   "fogColor+=directionalLight.color*pow(fDirDot,2.);"
   "PointLight pointLight=GetPointLight();"
 ...
 "{"
   ...
   "if(_TextId==1.)"
     "{"
       "vec2 _uv=uv;"
       "float off=distance(vec2(.5),vec2(uv.x))*.005;"
       "vec4 d=vec4(hash(uv),hash(uv*.75));"
       "vec2 rnd=vec2(rand(uv+d.x*.05),rand(uv+d.z*.05));"
       "const vec2 lensRadius=vec2(1.1375,.05);"
       "float dist=distance(uv.xy,vec2(.5,.5)),vigfin=pow(1.-smoothstep(lensRadius.x,lensRadius.y,dist),2.);"
       "rnd*=.025*vigfin+d.xy*2.*vigfin;"
       "vec4 text=texture(_TextTex,uv);"
       "uv+=rnd;"
       "text+=mix(texture(_TextTex,uv),vec4(1.),vec4(rnd.x))*.15;"
       "return vec4(text.xyz,off+.01+.005*cos(_uv.x*.1)*sin(_uv.y*.1));"
     "}"
   ...
   "if(_TextId==3.)"
     "{"
       ...
       "for(float i=1.;i<4.;i+=1.)"
         "text+=texture(_TextTex,uv+i*.0003).xyz;"
       "text/=4.;"
       "fragColor.xyz+=clamp(text,0.,1.);"
     "}"
   "if(_TextId==4.)"
     "{"
       "vec3 text=texture(_TextTex,uv*4.).xyz;"
       "for(float i=1.;i<4.;i+=1.)"
         "text+=texture(_TextTex,uv*4.+i*.0003).xxx;"
       "text/=4.;"
       "fragColor.xyz+=clamp(text,0.,1.);"
     "}"
   "return fragColor;"
